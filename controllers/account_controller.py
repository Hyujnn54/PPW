...
from utils.security import SecurityValidator, SecureMemory
...
class AccountController:
    ...
    def create_account(
        ...
        try:
            ...
            # Create account
            success, message = AccountService.add_account(
                user_id=user_id,
            ...
            SecureMemory.secure_delete(master_password)
    ...
    def get_account_details(
        ...
        try:
            ...
            if include_password:
                if not encryption_key and master_password:
                    encryption_key = MasterPasswordService.get_encryption_key(user_id, master_password)

                if encryption_key:
                    password = AccountService.get_password(user_id, account_id, encryption_key)
                    account_data['password'] = password

                    # Include notes if available
                    if account.get('notes_encrypted'):
                        from utils.encryption import EncryptionManager
                        notes = EncryptionManager.decrypt_data(
    ...
    def update_account(
        ...
        **updates
        ...
        try:
            ...
            # Update account
            success, message = AccountService.update_account(
                user_id=user_id,
            ...
            if 'password' in updates:
                SecureMemory.secure_delete(updates['password'])
            SecureMemory.secure_delete(master_password)
    ...
    def generate_strong_password(
        ...
        """
        Generate a strong password with analysis
        """
        try:
            # Validate length
            if length < 8:
